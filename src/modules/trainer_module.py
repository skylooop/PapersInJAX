...
from typing import Tuple, Callable, Iterator, Optional, Dict
import collections
import time
...
class TrainerModule:
    def __init__(self,
        ...
        self.init_model(dummy_input)
        
    def print_model(self, dummy_input: tp.Any):
        print(self.model.tabulate(jax.random.PRNGKey(0), *dummy_input, train=True))
        
    def init_logger(self, 
                    logger_params: tp.Optional[dict] = None):
        if logger_params is None:
            logger_params = dict()
        logging_dir = logger_params.get("log_dir", None)
        if not logging_dir:
            # if no path to logdir provided -> log into checkpoints dir
            ...
            version = ''
        logger_type = logger_params.get('logger_type', 'TensorBoard').lower() #default to TB
    ...
    def create_jitted_functions(self):
        '''
        Create jitted version of train/eval functions.
        '''
        train_step, eval_step = self.create_functions()
        if self.debug:
            print("No jitting in debug mode")
    ...
    def init_model(self, dummy_input: tp.Any):
        ...
        variables = self.run_model_init(dummy_input, init_rng)
    ...
    def run_model_init(self, dummy_input: tp.Any,
                       init_rng: tp.Any) -> tp.Dict:
        return self.model.init(init_rng, *dummy_input, train=True)
    ...
    def create_functions(self) -> Tuple[Callable[[TrainState, tp.Any], Tuple[TrainState, tp.Dict]],
                                        Callable[[TrainState, tp.Any], Tuple[TrainState, tp.Dict]]]:
        """
    ...
    def train_model(self,
                    ...
                    num_epochs : int = 500) -> Dict[str, tp.Any]:
        """
    ...
    def train_epoch(self,
                    train_loader : Iterator) -> Dict[str, tp.Any]:
        """
    ...
    def eval_model(self,
                   ...
                   log_prefix : Optional[str] = '') -> Dict[str, tp.Any]:
        """
    ...
    def is_new_model_better(self,
                            new_metrics : Dict[str, tp.Any],
                            old_metrics : Dict[str, tp.Any]) -> bool:
        """
